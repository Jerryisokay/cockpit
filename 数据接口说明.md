...
| themeType          | 默认风格主题类型， 对应：0 暗色主题，1 粉色主题，2 深蓝色主题， 3 蓝白色主题 |
| **id**             | **平台id，用于区分不同平台的可视化系统**                     |
| **hideNav**        | **是否隐藏顶部导航，默认 false 不隐藏, true 隐藏**           |
...
| ~~size~~      | ~~可选，图表尺寸大小： 0 小尺寸，1 较大尺寸，为了排版美观，图表的宽度都是一样的，大小尺寸对应的高度分别是 400px 和 200px，目前已不需要维护~~ |
| id            | 图表唯一标识，用于获取单个图表的数据                         |
| width         | 必选，图标框的宽度方格数，页面显示区域分成36*36的方格        |
| height        | 必选，图标框的高度方格数                                     |
| positionX     | 必选，图标框的横坐标方格数，以左上角为原点                   |
| positionY     | 必选，图标框的纵坐标方格数                                   |
| **moreUrl**   | **更多链接 ，用于跳转，目前仅用于表格(type = 20)和按钮(type = 25)图表** |
| **refresh**   | **图表的刷新时间，毫秒数**                                   |
| colors        | 可选，示例：`['#ff9600','#ff8100']`  主要颜色列表，如果为空，或者颜色数量少于数据组数，就显示前台默认颜色 |
| type          | 必选，图表类型：0 单系列饼图，1 多系列饼图，2 多系列百分比饼图， 3 单系列横向柱形图，4  单系列纵向柱形图，5 多系列横向柱形图，6 多系列纵向柱形图，7 折线图, 8 数据面积图， 9 柱形折线混合图， 10 雷达图，11 漏斗图，12 仪表盘图，13 树图，14  矩形树图，15 旭日图，16 散点图，17 气泡图，18 主题河流图 ，19 日历坐标系 ， 20 表格，21统计表， 22 设备管理图表，23 iframe 内嵌网页，24 video视频， 25 按钮组图表，26 幻灯片组   <br />  **具体示例见下面附注** |
| ~~multiple~~  | ~~必选，是否是多系列数据，0 单系列，1 多系列 ，**具体参考下面附注~~** |
...
| equipmentData | 设备管理图数据，对应type = 22 时候需要，**具体示例见下面附注** |
| **gallerys**  | **幻灯片组数据，对应type = 26 时候需要，具体示例见下面附注** |
| **url**       | **链接，不同的图表类型下用途不同**                           |
| **btnData**   | **按钮数据，对应type = 25， 每个按钮可由自定义图标和文字组成，点击执行自定义的脚本，具体示例见下面附注** |
...
```js
{
      ...
      calendarData: null,
      equipmentData:[
        /* type： 0 用于显示数字，1 用于显示状态， 2  用于显示报警 */
        ...
        { "num1":"PM25" , "name":"PM25" , "type":0 , "value":"113.44"  },
        { "num1":"PM1" , "name":"PM1" , "type":0 , "value":"160.113"  }, 
      ]
}
```
...
在图表区域内播放外链多媒体视频（rtmp），注意图表显示区域的实际宽高比，宽/高 比例不要大于视频源的宽/高比例。如超出显示区域，请适当减小图表宽度（width）或增加高度（height）
...
用于显示由自定义组成的按钮图表，按钮点击执行自定义脚本或跳转链接
...
`scriptType`  脚本类型，对应： 0   js脚本， 1   跳转链接（window.open方式打开） 

`script`   脚本内容的字符串， 如果是执行 js脚本，注意作用域，如果执行父级页面的js方法，注意使用 parent 节点
...
1. <https://www.iconfont.cn/> 在阿里图标库新建项目，参考截图 `example/icon_demo1.png`， 项目的`font-famliy`  设置为`iconbtn` ,` class` 伪类的前缀也为 `iconbtn-`
2. 项目中新增的图标参考截图 `example/icon_demo2.png`， 设置图标的 `class` , 例如 `setting` ， 则这个图表的完整`class`伪类名就是`iconbtn-setting`
3. 图标全部新增完毕后，点击项目中的下载至本地按钮，参考截图 `example/icon_demo1.png` ， 解压压缩包，把其中的 `iconfont.css`, `iconfont.eot`, `iconfont.svg`, `iconfont.ttf`, `iconfont.woff`, `iconfont.woff2` 放到 `src/styles/buttons/` 文件夹下替换即可，**注意不要放错，不要替换`styles/`下的文件，这里的图标是项目里面其他的，不要替换错** 
4. 接口数据中传递的图标必须是打包的图标库里已有的，直接传递图标class名中 `iconbtn-` 后面的部分即可，参考第2条，如`iconbtn-setting` , 接口数据中给的 icon只需要 `setting` 就好
